...
const NewCategory = () => {
    ...
    const {color, setColor,setCategories,categories}=useContext(GlobalContext)
    const [expOrIncome, setexpOrIncome] = useState('both');
    return ( 
        <>
            <div className="bg-white m-2 flex flex-1 rounded-md flex-col shadow-xl ">
                ...
                <form className="flex justify-start flex-col p-4 h-full" >
                    ...
                    <input type="text" placeholder="name" className="rounded-md border border-gray-500 m-2"/>
                    
                    <div className="flex-1 flex justify-around text-gray-800 text-sm">
                    <div className="flex items-center"><input type="checkbox" className=""/><span>incomes</span></div>
                    <div className="flex items-center"> <input type="checkbox" name="expencies only" /> <span>expeinces </span> </div>
                    </div>
                    ...
                    <button type="submit" 
                    className={`bg-green-600 text-lg  rounded-md text-white`}
                    onClick={(e)=>{
                        e.preventDefault()
                        const newCateg = {
                            name:name,
